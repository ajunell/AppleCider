...
def get_model(num_classes, config):
    model = Informer(enc_in=config['p_feature_size'], d_model=config['p_d_model'], dropout=config['p_dropout'], factor=1,
                     output_attention=False, n_heads=config['p_n_heads'], d_ff=config['p_d_ff'],
                     activation='gelu', e_layers=config['p_encoder_layers'], seq_len=config['seq_len'],
    ...
    if config['use_pretrain']:
        if config['use_pretrain'].startswith('CLIP'):
            ...
            filtered_weights = {k[len('photometry_encoder.'):]: v for k, v in weights.items() if
                                k.startswith('photometry_encoder')}
