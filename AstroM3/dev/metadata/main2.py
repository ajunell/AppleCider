...
from dataset2 import VPSMDatasetV2Meta
...
def get_model(num_classes, config):
    model = MetaModel(num_classes=num_classes, hidden_dim=config['m_hidden_dim'], dropout=config['m_dropout'])
    ...
    if config['use_pretrain']:
        ...
        if config['use_pretrain'].startswith('CLIP'):
            ...
            filtered_weights = {k[len('metadata_encoder.'):]: v for k, v in weights.items() if
                                k.startswith('metadata_encoder')}
            model.load_state_dict(filtered_weights, strict=False)

            not_loaded_keys = [k for k in model.state_dict().keys() if k not in filtered_weights]
            print('Keys not loaded:', not_loaded_keys)
