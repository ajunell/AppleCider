...
import numpy as np
import tensorflow as tf ; TF_FORCE_GPU_ALLOW_GROWTH=True
import torch
...
class DataGenerator(tf.keras.utils.Sequence):
    '''
    for spectra and non-spectra
    '''
    def __init__(self, preprocessed_path, step, batch_size=32, spectra=True, shuffle=True, file_list=None, **kwargs):
    ...
    def __getitem__(self, index):
        ...
        batch_x = tuple(tf.ragged.constant(item) for item in zip(*batch_x))
    ...
    def _load_data(self, filename):
        ...
        if self.spectra == True :
            # get spectra csv, save wavelengths fluxes
            obj_id_alert = str(filename)
            obj_id = obj_id_alert[:12] # drops everything but ZTFID# from ZTFID#_alerts.npy file name 
            ...
            X = (sample['photometry'], sample['metadata'].to_numpy(), sample['images'], spectra_df.values)
